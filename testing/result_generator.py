...
from scripts.utils.bill_parser import get_alpha_numeric_string
...
def generate_test_results():
    ...
    all_results = pd.DataFrame(columns=["EXTRACTION_MODEL", "OCR_MODEL", "ACCURACY", "ACCURACY_STD"])
...
def compare_results(
    ...
    for i, gt_row in gt_dataset.iterrows():
        ...
        accuracy_result = get_accuracy_results(gt_row, compare_data)
        results.append({
...
def get_accuracy_results(gt_row: pd.Series, compare_data: dict) -> float:
    ...
    for gt_key, dict_key in gt_to_dict.items():
        ...
        if gt_key != "FECHA":
            ...
            try:
                gt_data = pd.to_datetime(gt_data, dayfirst=True).strftime("%d-%m-%Y")
            except Exception:
                accuracy.append(0)
                continue
...
def generate_test_result_one_shot():
    ...
    all_results = pd.DataFrame(columns=["EXTRACTION_MODEL", "ACCURACY", "ACCURACY_STD"])
